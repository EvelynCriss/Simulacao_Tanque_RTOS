...
exercicio_06_split4_codigo_unificado -> este arquivo esta com o codigo todo unificado, caso precise
...
Supervisão	Periódica e Bloqueada	vTaskDelayUntil() e xSemaphoreTake()
Emergência	Bloqueada	vTaskDelay() por 10 segundos
Reset	Bloqueada	Verificação periódica utilizando vTaskDelay()
...
A task Emergência aguarda 10 segundos.
Libera o semáforo semEmergencia.
A task Supervisão detecta o evento.
O sistema entra em MODO_SEGURO.
Um comando de proteção é enviado para a fila dos atuadores.
...
No código atual, a task Sensores é responsável por atualizar o estadoTanque. O mutex foi utilizado para garantir acesso seguro ao recurso compartilhado e permitir expansão futura do sistema.
...
Supervisão detecta a falha.
Sistema entra em MODO_SEGURO.
Supervisão muda o estado para MODO_AGUARDANDO_RESET.
A task Reset detecta esse estado.
A task Reset aguarda 5 segundos, simulando a verificação do operador.
A task Reset libera semReset.
Supervisão recebe o reset e retorna para MODO_NORMAL.
...
Adicione os arquivos .c e .h ao projeto FreeRTOS.
Certifique-se de que os includes do FreeRTOS estejam configurados corretamente.
Compile o projeto normalmente no ambiente utilizado.
Execute a função principal da simulação:
